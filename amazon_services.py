...
secret_key = '7AEiabc2lNwlyrjXCjCgtIafNB9xNiBUD3ZxAAlD' #replace with your secret key
...
env = Environment(loader=FileSystemLoader('/home/brentp/Projects/boto_test/templates'),
                  trim_blocks=True,
                  lstrip_blocks=True)
...
def sendFeed(feed_type, feed_xml):
    ...
    # send feed
    feeds = mws.Feeds(access_key=access_key, secret_key=secret_key, account_id=merchant_id)
...
def getFeedStatus(feed_id):
        ...
        info =  submission_list.find('GetFeedSubmissionListResult').FeedSubmissionInfo
        status = info.FeedProcessingStatus.string 
        return status  
        
def getOrders(last_date):
    orders = mws.Orders(access_key=access_key, secret_key=secret_key, account_id=merchant_id)
    marketplace_ids = (marketplace_id,)
    order_info = orders.list_orders( marketplace_ids, lastupdatedafter=last_date)
    return order_info
...
def get_book_info(asin):    
    ...
    if book_info.find('Item'):
        ...
        processAmazonBook(book_info.find('Item'), False)
    else:
        print(book_info.prettify())
...
def get_book_info_2(asins):    
    ...
    products = mws.Products(access_key=access_key, secret_key=secret_key, account_id=merchant_id)
    result = products.get_matching_product_for_id(marketplace_id, 'ASIN',asins)
...
def get_book_price_info(asins, condition):    
    ...
    products = mws.Products(access_key=access_key, secret_key=secret_key, account_id=merchant_id)
    result = products.get_lowest_offer_listings_for_asin(marketplace_id, asins, condition=condition)
